...
public class Application {
	...
	public static URLCreator urlCreator = new URLCreator();
	public static JSONHelper jsonHelper = new JSONHelper();
	public static Schema schema;
	
	public static Cache cacheLT50 = new Cache();
	public static Cache cacheLT100 = new Cache();
	public static Cache cacheLT200 = new Cache();
	public static Cache cacheMT200 = new Cache();
	...
	public static void main(String[] args) {
		...
		result = dataProvider.query("http://api.zappos.com/Search/term/all?limit=100&facets=[%22priceFacet%22]&excludes=[%22results%22]&key=52ddafbe3ee659bad97fcce7c53592916a6bfd73");
		...
		schema = new Schema(lt50, lt100, lt200, mt200);
		
		/* Get the input from the user */
		Scanner keyboard = new Scanner(System.in);
		int numberOfItems;
		double targetPrice;
		System.out.print("Please enter the total number of items you would like: ");
		numberOfItems = keyboard.nextInt();
		System.out.print("Please enter the total price you would like to pay: ");
		targetPrice = keyboard.nextDouble();
		
		/* Randomly assign values to the items and find items close to their price in a search */
		int i;
		...
		for (i = 0; i < numberOfItems; i++) {
			...
			if (remaining / (numberOfItems - i) > 6) {
				tmp = (double)rand.nextInt((int)(remaining - (6 * (numberOfItems - i))));
	...
	public static Product binaryAPICalls(double price, int range) {
		...
		CacheEntry highCache, lowCache, currentCache;
		
		numberOfPages = schema.getNumberOfPages(range);
		...
		while ((!(price <= currentCache.getMax() && price >= currentCache.getMin())) 
	...
	public static Product binaryArraySearch(double price, CacheEntry cache) {
		int highPos = 0;
		int currentPos = 50;
		int lowPos = 99;
		JSONArray array = jsonHelper.extractArrayFromString(cache.getData(), "results");
		double currentPrice = getProductPrice(currentPos, array);
		while ((currentPrice != price) && ((currentPos - 1 != highPos) && (currentPos + 1 != lowPos))) {
	...
	public static double parsePrice(Object obj) {
		return Double.parseDouble(((String)((JSONObject)obj).get("price")).replaceAll("[^\\d.]+", ""));
	}
	...
	public static Cache getCache(int range) {
		switch(range){
		case 50:
			return cacheLT50;
		case 100:
	...
	public static CacheEntry createCacheEntry(int currentPage, int range){
		String url, result;
		JSONArray array;
		Object obj;
		double max, min;
	...
	public static Product createProduct(int pos, JSONArray array) {
		...
		rv.originalPrice = Double.parseDouble(((String)((JSONObject)obj).get("originalPrice")).replaceAll("[^\\d.]+", ""));
		rv.percentOff = (String)(((JSONObject)obj).get("percentOff"));
		rv.productId = Integer.parseInt((String)(((JSONObject)obj).get("productId")));
		rv.styleId = Integer.parseInt((String)(((JSONObject)obj).get("styleId")));
		rv.colorId = Integer.parseInt((String)(((JSONObject)obj).get("colorId")));
